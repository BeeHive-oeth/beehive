 ...
 * Hive
 * Copyright (C) 2020 Hive Protocol
 *
...
import "./HiveOptions.sol";


/**
 * @author 0mllwntrmt3
 * @title Hive OETH Call Options
 * @notice OETH Call Options Contract
 */
contract HiveCallOptions is HiveOptions {
    HiveOETHPool public pool;

    /**
     * @param _priceProvider The address of ChainLink OETH/USD price feed contract
     */
    constructor(AggregatorInterface _priceProvider)
        public
        HiveOptions(_priceProvider, HiveOptions.OptionType.Call)
    {
        pool = new HiveOETHPool();
    }
    ...
    /**
     * @notice Sends premiums to the OETH liquidity pool contract
    ...
    /**
     * @notice Sends profits in OETH from the OETH pool to a call option holder's address
     * @param option A specific option contract
     */
