...
contract("HiveCallOptions", ([user1, user2, user3, user4]) => {
  ...
  it("Should be the owner of the pool contract", async () => {
    const {CALL, OETHPool} = await contracts
    assert.equal(
      await OETHPool.owner(),
      CALL.address,
  ...
  it("Should provide funds to the pool", async () => {
    ...
    await OETHPool.provide(0, {value, from: user4})
  })
  ...
  it("Shouldn't change pool's total amount when creates an option", async () => {
    const {OETHPool} = await contracts
    const startTotalBalance = await OETHPool.totalBalance()
    const amount = Math.random().toFixed(18)
    const strike = new BN(200e8)
    const {id} = await createOption({amount: toWei(amount), strike})
    const endTotalBalance = await OETHPool.totalBalance()
  ...
  it("Shouldn't change users' share when creates an option", async () => {
    ...
    const startShares = await Promise.all(
      [user1, user2, user3].map((user) => OETHPool.shareOf(user))
    ).then((x) => x.toString())
    const amount = Math.random().toFixed(18)
    const strike = new BN(200e8)
    const {id} = await createOption({amount: toWei(amount), strike})
  ...
  it("Should unfreeze LP's profit correctly after an option is unlocked", async () => {
    const {OETHPool, CALL} = await contracts
    const startTotalBalance = await OETHPool.totalBalance()
    const amount = Math.random().toFixed(18)
    const strike = new BN(200e8)
    const {id} = await createOption({amount: toWei(amount), strike})
  ...
  it("Should withdraw funds from the pool", async () => {
    const {DAI, OETHPool} = await contracts
    const value = await OETHPool.availableBalance()
    await timeTravel(14 * 24 * 3600 + 1)
    // await OETHPool.lockupPeriod().then(timeTravel)
    await OETHPool.withdraw(value, "100000000000000000000000000000000", {from: user4})
  })
