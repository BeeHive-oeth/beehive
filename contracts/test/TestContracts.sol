...
contract FakeExchange {
    ...
    function swapExactETHForTokens(
        ...
        uint256 amount = getOETHToTokenInputPrice(msg.value);
    ...
    function getTokenToOETHInputPrice(uint256 tokens_sold)
        public
        view
        returns (uint256 OETH_bought)
    {
        OETH_bought =
    ...
    function getOETHToTokenInputPrice(uint256 OETH_sold)
        public
        view
        returns (uint256 tokens_bought)
    {
        tokens_bought =
            (OETH_sold * uint256(price.latestAnswer()) * spread) /
            1e10;
    }
